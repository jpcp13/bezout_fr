...
function plots(bez::Array{Float64, 3}, beztri::Array{Float64, 3}, B::Array{Float64, 3}, i_start::Int)
    ...
    savefig("../png/diagR.png")
    clf()
    tbb = triang_block(beztri)
    a = (tbb.A)[:,:,n+1]
    plot(log10(1e-16 + abs(diag(a, tbb.r))), "r*")
    grid("on")
    savefig("../png/diagRtri.png")
    clf()
    plot(log10(1e-16 + abs(diag(flipdim(B[:, :, n+1], 2), i_start))), "r*")
    grid("on")
    savefig("../png/diagRfinal.png")
end

function bloc_qrp_plot(B0::Array{Float64, 2}, fn::Int, s::Int)
	dR = zeros(0)
	for k = 1:fn
		Bk = B0[k*s-s+1:k*s, k*s-s+1:k*s]
		F = qrfact(Bk, Val{true})
		Q, R, p = F[:Q], F[:R], F[:p]
		dR = [dR; diag(R)]
	end
	return dR
