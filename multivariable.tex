...
\begin{algorithm}[H]
\SetAlgoLined
\KwData{$\vecb{d}$, multidegré $(d_1, \cdots, d_n)$ d'un système de polynômes $f$.}
\KwResult{$U$ et $V$ ensembles de points de Frourier utilisés pour l'évaluation du polynôme de Bezout}
initialization\;
...
\caption{Création des ensembles points de Fourier $U$ et $V$}
\end{algorithm}

\begin{algorithm}[H]
\SetAlgoLined
\KwData{$\vecb{f}$, système $(f_1, \cdots, f_n)$ de $n$ polynômes de $\C^n$, de multidegré $(d_1, \cdots, d_n)$; $\vecb{k}$, entier compris entre $0$ et $n$.}
\KwResult{$\vecb{C_k}$, matrice indéxée par $(u, v) \in U\times V$ et contenant les évaluations $\delta(x_k)(u, v)$ }
initialization\;
\While{not at end of this document}{
read current\;
\eIf{understand}{
