...
\State $C_k \gets \textsc{zeros}(D, D)$
\For{$(u, v) \in U\times V$}
      \State $\Delta(x_k) \gets \textsc{zeros}(n, n)$
   		\For{$i, j=1..n$}
      		\State $\Delta(x_k)(i, j) \gets \delta_{i,j}(x_k)(u, v)$ \Comment{$\delta_{i,j}(x_k)$ défini à la formule (\ref{finite_diff})}
   		\EndFor
		\State $C_k(u, v) \gets \textsc{det}(\Delta(x_k))$
	\EndFor
...
Tandis que l'on a $C^k_{u,v} = \delta(x_k)(u, v)$, la matrice de Bezout $B(x_k) = \left[b_{\alpha\beta}\right]$ est définie par le développement $\delta(x_k) = \sum_{\alpha,\beta} b_{\alpha\beta} x^\alpha y^\beta$. Ceci montre que $C^k$ est la transformée de Fourier discrète de la matrice $B_k$, c'est-à-dire que si $F_U$ désigne la matrice $F_u = \left[ u^\alpha \right]_{u\in U, \alpha \in D}$
