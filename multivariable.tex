...
\For{$(u, v) \in U\times V$}
      \State $m \gets \textsc{zeros}(n, n)$
   		\For{$i, j=1..n$}
      		\State $m(i, j) \gets \delta_{i,j}(x_k)(u, v)$ \Comment{$\delta_{i,j}(x_k)$ défini à la formule (\ref{finite_diff})}
   		\EndFor
		\State $C_k(u, v) \gets \textsc{det}(m)$
	\EndFor
